...
class OrderBookAnalysis:
    ...
    def __init__(self):
        ...
        self.book_score: float = 0.0  # -1..1 microstructure signal
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "book_score": round(self.book_score, 4),
        }
...
class OrderBookAnalyzer:
    ...
    def analyze(
        ...
        analysis.book_score = self._compute_book_score(analysis)
    ...
    def _compute_book_score(self, analysis: OrderBookAnalysis) -> float:
        """
        Compute a directional microstructure score (-1..1).

        Combines OBI, depth ratio, and pressure, weighted by liquidity quality
        and penalized when spoofing or wide spreads are detected.
        """
        # Directional components
        obi_score = float(np.clip(analysis.obi, -1.0, 1.0))
        depth_ratio = analysis.depth_ratio if analysis.depth_ratio > 0 else 1.0
        depth_score = (depth_ratio - 1.0) / (depth_ratio + 1.0)

        pressure_score = 0.0
        if analysis.pressure == "bullish":
            pressure_score = 0.5
        elif analysis.pressure == "bearish":
            pressure_score = -0.5

        whale_total = len(analysis.whale_bids) + len(analysis.whale_asks)
        whale_bias = 0.0
        if whale_total > 0:
            whale_bias = (len(analysis.whale_bids) - len(analysis.whale_asks)) / whale_total

        directional = (0.6 * obi_score) + (0.25 * depth_score) + (0.1 * pressure_score) + (0.05 * whale_bias)

        # Quality adjustment
        quality = analysis.liquidity_score if analysis.liquidity_score > 0 else 0.5
        if analysis.spoofing_detected:
            quality *= 0.5
        if analysis.spread_pct > 0.003:
            quality *= 0.7

        return float(np.clip(directional * quality, -1.0, 1.0))
