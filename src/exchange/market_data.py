...
class MarketDataCache:
    ...
    async def update_bar(self, pair: str, bar: Dict[str, float]) -> bool:
        """
    ...
    def update_order_book_analysis(self, pair: str, analysis: Dict[str, Any]) -> None:
        """Update latest order book analysis snapshot."""
        self._order_book_analysis[pair] = {
            **analysis,
            "updated_at": time.time(),
        }
    ...
    def get_times(self, pair: str, n: Optional[int] = None) -> np.ndarray:
        return self._get_col(pair, self.COL_TIME, n)
    ...
    def get_order_book_analysis(self, pair: str) -> Dict[str, Any]:
        return self._order_book_analysis.get(pair, {})
