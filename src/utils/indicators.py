...
def compute_sl_tp(
    ...
    sl_mult: float = 2.25, tp_mult: float = 3.0,
...
def volume_ratio(volumes: np.ndarray, period: int = 20) -> np.ndarray:
    ...
    Avoids divide-by-zero so no RuntimeWarning on Pi.
    """
    avg_vol = sma(volumes, period)
    result = np.ones_like(volumes, dtype=float)
    valid = (~np.isnan(avg_vol)) & (avg_vol > 0)
    np.divide(volumes, avg_vol, out=result, where=valid)
    return result
