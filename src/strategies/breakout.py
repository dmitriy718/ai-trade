    ...
    async def analyze(
        ...
        **kwargs
        ...
        if cache:
            ...
            vol_ratio_vals = cache.volume_ratio(20)
        ...
        if cache:
            upper, middle, lower = cache.bollinger_bands(20, 2.0)
        else:
        ...
        if direction == SignalDirection.LONG:
            stop_loss, take_profit = compute_sl_tp(
                curr_price, curr_atr, "long", 2.5, 4.0, round_trip_fee_pct=fee_pct
            )
        elif direction == SignalDirection.SHORT:
            stop_loss, take_profit = compute_sl_tp(
                curr_price, curr_atr, "short", 2.5, 4.0, round_trip_fee_pct=fee_pct
            )
        else:
            stop_loss = 0.0
            take_profit = 0.0
